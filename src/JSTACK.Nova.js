JSTACK.Nova = (function(JS, undefined){
	...
	var _check = function() {
		if (params.url != undefined) return true;
	...
	var getserverlist = function(detailed, callback) {
		...
		JS.Comm.get(url, JS.Keystone.params.token, 
				_onOK, _onError);
	}
	...
	var updateserver = function(id, name, callback) {
		...
		JS.Comm.put(url, data, JS.Keystone.params.token,
				_onOk, _onError);
	}
	
	var createserver = function(name, imageRef, flavorRef,
                           key_name, user_data, security_groups,
                           min_count, max_count, availability_zone, 
                           callback) {
		if (!_check()) return;
		var data = {
    		"server" : {
		...
		if (security_groups != undefined) {
			...
			for (var i in security_groups) {
				var group = {"name" : security_groups[i]};
				groups.push(group);
			}
		...
		JS.Comm.post(params.url + '/servers', data, 
				JS.Keystone.params.token, _onOK, _onError);
	...
	var deleteserver = function(id, callback) {
		...
		JS.Comm.del(url, JS.Keystone.params.token, 
				_onOk, _onError);
	}
	...
	var _postAction = function(id, data, callback) {
		...
		JS.Comm.post(url, data, JS.Keystone.params.token, 
				_onOk, _onError);
	}
	
	var changepasswordserver = function(id, adminPass, callback) {
		if (adminPass == undefined) return;
	...
	var rebootserverhard = function(id, callback) {
		_postAction(id, {"reboot":{"type":"HARD"}}, callback);
	}
	
	var rebootserversoft = function(id, callback) {
		_postAction(id, {"reboot":{"type":"SOFT"}}, callback);
	}
	
	var resizeserver = function(id, flavorRef, callback) {
		_postAction(id, {"resize":{"flavorRef":flavorRef}}, callback);
	}
	
	var confirmresizedserver = function(id, callback) {
		_postAction(id, {"confirmResize":null}, callback);
	}
